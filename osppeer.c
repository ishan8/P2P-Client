...
int calc_md5(char *c, char *dig) {
	char buf[2049]; 
	md5_state_t state;
	md5_init(&state);
	int size, f;
	if ((f = open(c, O_RDONLY))) {
		while (1) {
			size = (int) read(f, buf, 2048);
			buf[2048] = '\0';
			if (size == 0) { 
				size = md5_finish_text(&state, dig, 1);
				dig[size] = '\0';
				close(f);
				return size;
			}

			md5_append(&state, (md5_byte_t*) buf, size);
		}
	}
	return 0;
}
...
task_t *start_download(task_t *tracker_task, const char *filename)
{
	...
	assert(tracker_task->type == TASK_TRACKER);
 		...
 		if (strlen(filename) > 0) {
 			osp2p_writef(tracker_task->peer_fd, "MD5_CHECKSUM %s\n", filename);
 			messagepos = read_tracker_response(tracker_task);
 			s1 = tracker_task->buf;
 			s2 = memchr(s1, '\n', (tracker_task->buf + messagepos) - s1);
 			if (tracker_task->buf[messagepos] == '2') {
 				osp2p_snscanf(s1, (s2 - s1), "%s\n", tracker_task->md5_digest);
 				tracker_task->md5_digest[127] = '\0';
 				if (strlen(tracker_task->md5_digest) >= 5) {
 					message("* Valid checksum ('%s') for file '%s'\n", tracker_task->md5_digest, filename);
 				} else {
 					strcpy(tracker_task->md5_digest, "");
 					message("* Invalid checksum: '%s'.\n", filename);
 				}
 			}
 		}
	#endif
...
static void task_download(task_t *t, task_t *tracker_task)
{
	...
	if (t->total_written > 0) {
		...
		#ifdef MD5_CHECKING
 			if (strlen(tracker_task->md5_digest) > 0) {
 				char chk[128];
 				if (calc_md5(t->disk_filename, chk) != 0) {
 					if (strcmp(chk, tracker_task->md5_digest) == 0) {
 						message("* Match: '%s'!\n", t->filename);
 					} else {
	 					message("* Mismatch: '%s'.\n", t->filename);
